 ...
 */
public class ThTrovaU extends Thread {
    ...
    public ThTrovaU(boolean useSleep, boolean useYield, String s, DatiCondivisi ptrdati, Semaforo mutex, Semaforo visualizza, Semaforo aspetta) {
     ...
     * TrovaU quando viene richiamato il metodo start presente nella classe
     * Thread.
     *
     */
    public void run() {
     ...
     * 'u' e 'U' ed in base ai valori di useSleep e useYield richiama i metodi
    ...
    public void TrovaU() {
        ...
        for (int i = 0; i < frase.length(); i++) {
            ...
            if (frase.charAt(i) == 'U' || frase.charAt(i) == 'u') {
                mutex.Wait();
                conta = ptrdati.getCountU();
                mutex.Signal();
                conta++;
                mutex.Wait();
                ptrdati.setCountU(conta);
                mutex.Signal();
    ...
    public void usaSleep() {
        try {
            ...
            Logger.getLogger(ThTrovaU.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
